...
namespace MediaWiki\Extension\ForTraining;
...
class Hooks {
	...
	public static function onResourceLoaderGetConfigVars(array &$vars) {
		global $wgForTrainingGenerateOdtUrl;

		if (isset($wgForTrainingGenerateOdtUrl)) {
			$vars['wgForTrainingGenerateOdtUrl'] = $wgForTrainingGenerateOdtUrl;
		}
	...
	public static function onSkinTemplateNavigation(\SkinTemplate $skinTemplate, array &$links ) {
		...
		if ( $title->getNamespace() !== NS_SPECIAL && $wgUser->isLoggedIn()) {
			...
			$allowed = array("God's Story (five fingers)", "God's Story (first and last sacrifice)", 'Baptism',
				...
				'Church', 'Healing', 'My Story with God', 'Bible Reading Bookmark (Seven Stories full of Hope)');	// Only enable the function for these worksheets (temporary, TODO)*/
			if (!in_array($worksheet, $allowed))
				return true;
				
			// important: load our javascript class
			$skinTemplate->getOutput()->addModules( 'ext.forTraining' );
