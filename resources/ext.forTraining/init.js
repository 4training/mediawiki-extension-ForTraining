( function() {
	if (!mw.config.exists('wgForTrainingGenerateOdtUrl')) {
		mw.notify('Missing configuration variable $wgForTrainingGenerateOdtUrl. ODT-Generator not available.', { title: 'Error' })
		return;
	}
    $( "#ca-generateodt a" ).on( 'click', function ( e ) {
		...
		$.post(mw.config.get('wgForTrainingGenerateOdtUrl'), postArgs)
			.done( function( data ) {
